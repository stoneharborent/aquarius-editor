...
export default {
  appId: 'os.aquarius.cut',
  productName: 'Aquarius Cut',
  // The product name has a space in it, so the file name is spelled out here instead of
  // ${productName}: release files stay shell-friendly (AquariusCut-0.2.11-arm64.dmg).
  artifactName: 'AquariusCut-${version}-${arch}.${ext}',
  ...
  // hand OpenChatCut's releases to Aquarius Cut users as if they were ours. Aquarius Cut has
  // no GitHub home of its own yet, so packaging simply does not publish and does not emit
  // auto-update metadata (latest-*.yml / .blockmap). The renderer and the Electron updater
  // are switched off to match — see src/ui/upstreamUpdate.ts and desktop/update-service.ts.
  //
  // To turn updates back on once the repo has a home, restore all four pieces together:
  ...
  mac: {
    ...
    icon: 'assets/branding/aquarius-cut-icon.icns',
  ...
  win: {
    target: ['nsis'],
    icon: 'assets/branding/aquarius-cut-icon.ico',
  },
  nsis: {
    oneClick: false,
  ...
  linux: {
    ...
    category: 'AudioVideo',
    // Keep the executable name stable for release/linux-unpacked/aquarius-cut and CI smoke tests.
    executableName: 'aquarius-cut',
