 ...
 * Aquarius Editor timeline (independent absolute frame bits for each track) to FCPX magnetic timeline (connected clips with lane)
...
export function timelineToFcpxml(
  ...
  const title = escapeXml((opts.title ?? '').trim() || 'Aquarius Editor Timeline');
  ...
  const backgroundFillWarning = fcpxmlBackgroundFillCount(state) > 0
    ? xmlComment('WARNING: backgroundFill settings are preserved as Aquarius Editor metadata, but this exporter does not synthesize a portable blurred layer; render a video master to preserve the exact appearance.')
    : '';
  ...
  const eventName = nle === 'fcp_xml_resolve' ? 'Aquarius Editor Export (Resolve)' : 'Aquarius Editor Export';
