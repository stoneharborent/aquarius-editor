...
const llmPage = (preset: (typeof LLM_PROVIDER_PRESETS)[number]): SettingsVendorPage => {
  ...
  return {
    ...
    note: preset.id === 'anthropic'
      ? 'The built-in Agent requires an Anthropic API key. Claude Code subscription users should connect through “External agents (MCP)”; Aquarius Editor does not accept Claude OAuth.'
...
const CODEX_PAGE: SettingsVendorPage = {
  ...
  title: 'OpenAI · Codex',
  connection: 'codex',
  note: 'Sign in with a ChatGPT subscription. The official Codex CLI manages credentials, renewal, and logout; Aquarius Editor never reads or displays OAuth credentials.',
  fields: [
    {
...
const XAI_OAUTH_PAGE: SettingsVendorPage = {
  ...
  note: 'Sign in with your SuperGrok or X Premium+ subscription: the official Grok CLI owns login and credentials (run grok login in a terminal); Aquarius Editor imports the session and refreshes it automatically, and never reads or displays OAuth credentials.',
  fields: [{
