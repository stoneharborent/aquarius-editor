...
export function HyperframesSetupCard({ onSaved, compact, upgrade, problem }: HyperframesSetupCardProps) {
  const t = useT();
  // 'model-missing' is the ordinary case now, not a fault: the installer stopped
  // carrying the weights in v0.6.0 because they no longer fit in a release, so a
  // fresh install has no built-in model and reinstalling would not produce one.
  ...
  const problemText = problem === 'model-corrupt'
    ...
    : problem === 'runtime-unavailable'
      ...
      : problem === 'model-missing'
        ? t('This install does not include a built-in model, so graphics are generated by whichever provider you connect here. A local runtime (Ollama or LM Studio) needs no key and no account.')
        : null;
