...
// ── Configuration detection: keys, and local runtimes without keys ───────────
{
  const none = resolveHyperframesLlm(() => '');
  assert.equal(none.configured, false, 'a fresh install has nothing to generate with');

  const keyed = resolveHyperframesLlm((name) => (
    name === 'LLM_PROVIDER' ? 'anthropic' : name === 'LLM_ANTHROPIC_API_KEY' ? 'sk-test' : ''
  ));
  ...
  const local = resolveHyperframesLlm((name) => (name === 'LLM_PROVIDER' ? 'ollama' : ''));
  assert.equal(local.configured, true, 'a local runtime is configured without an API key');
  assert.equal(local.provider, 'ollama');
}

console.log('hyperframes.verify: generation, repair loop, validation and configuration detection OK');
