...
/** System Codex home that Aquarius Editor's isolated CODEX_HOME inherits credentials from. */
...
export class CodexAppServerClient {
  ...
  private async startProcess(): Promise<void> {
    ...
    try {
      await this.rawRequest('initialize', {
        clientInfo: { name: 'openchatcut', title: 'Aquarius Editor', version: '1' },
  ...
  private handleServerRequest(id: number | string, method: string, params: Record<string, unknown>): void {
    ...
    if (method === 'execCommandApproval' || method === 'applyPatchApproval') {
      this.writeMessage({ id, result: { decision: { denied: { rejection: 'Use the Aquarius Editor proposal review.' } } } });
      return;
    }
    ...
    for (const handler of this.serverRequests) {
      try {
        ...
        request.reject(-32603, 'Aquarius Editor could not handle this request.');
        return;
      }
    }
    request.reject(-32601, 'Method not supported by Aquarius Editor.');
  }
