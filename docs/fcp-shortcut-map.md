...
| Paste Effects (`paste-effects`) | Mod + Alt + V / Mod + Shift + B | **Mod + Alt + V** | ⇧⌘V ("Paste Attributes") | Deviation | The ⌘⇧B alias is **dropped** — B is blade territory now. ⌘⌥V is the plan's canonical choice (it is Premiere's Paste Attributes); FCP's own chord is ⇧⌘V, which is free and could be added as an alias if Royce wants strict parity. |
